...
#endif
	ZONE_MOVABLE,//内核定义的一个伪内存管理区，在防止物理内存碎片的机制memory migration中需要使用该内存域
...
struct zone {
	 ...
	 */
	unsigned long		min_unmapped_pages; // 存储了管理区内保留的页框的数目
	unsigned long		min_slab_pages;
	struct per_cpu_pageset	*pageset[NR_CPUS];
#else
	struct per_cpu_pageset	pageset[NR_CPUS];//数据结构用于实现单一页框的特殊高速缓存
#endif
	/*
...
#endif
	struct free_area	free_area[MAX_ORDER];//标示出管理区的空闲页框块
	 ...
	 */
	wait_queue_head_t	* wait_table;  //进程等待队列的三列表,这些进程正在等待管理区中的某页
	 ...
	 * rarely used fields:
	 */
	const char		*name; //指针指向管理区的传统名称,DMA,NORMAL,或HighMem
} ____cacheline_internodealigned_in_smp;
...
struct bootmem_data;
/* 节点(node)描述符
  * 系统的物理内存被划分为几个节点,每个节点的物理内存
  * 又分为几个管理区
  */
typedef struct pglist_data {
	struct zone node_zones[MAX_NR_ZONES];  //节点中管理区描述符的数组
	struct zonelist node_zonelists[MAX_ZONELISTS]; //页分配器使用的zonelist数据结构的数组,后备管理区
	...
	unsigned long node_present_pages; /* total number of physical pages ，不包括洞*/
...
#define NODE_DATA(nid)		(&contig_page_data)  // (UMA结构的机器)NODE_DATA指向全局的contig_page_data,对于NUMA结构的系统，NODE_DATA指向node_data
