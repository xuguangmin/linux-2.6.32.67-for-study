 ...
 */
 /* 页描述符
   * 内存页是物理内存管理中的最小单位,Linux会为系统为例内存每个页都创建一个
   * struct page对象，系统用一全局变量struct page *mem_map来存放所有
   * 物理页page对象的指针
   */
struct page {
	/* flags包含4部分section node zone flag.  其中标志位flag向高位增长，其余位向低位增长，中间存在空闲位
	  * section: 主要用于稀疏内存模型SPARSEMEM,可忽略
	  * node: NUMA节点号，表识该page属于哪一个节点
	  * zone: 管理区标志，标志该page属于哪一个zone
	  * flag: page的状态标志
	  */
	unsigned long flags;/* Atomic flags, some possibly
					 * updated asynchronously */
					 /*一组标志，也对页框所在的管理区进行标号,enum pageflags */
	atomic_t _count;		/* Usage count, see below. */ /*页框的引用计数器*/
	union {
 ...
 */
/* 它描述的是一段连续的、具有相同访问属性的虚存空间,
  * 它是访问MMAP地址空间的最小单元,该虚存空间的大小为
  * 物理内存页面的整数倍。
  */
...
struct mm_struct {
	...
	cpumask_t cpu_vm_mask;  // CPU下标,可存放系统中所有CPU下标
