...
struct inode {
	...
	struct list_head	i_dentry;	/*由于一个文件对应多个dentry，这些dentry都要链接到i_dentry这个链表头*/
	unsigned long		i_ino;		/*是inode的号*/
	atomic_t		i_count;	/*是inode的引用计数*/
	...
	umode_t			i_mode;		/*代表文件类型S_IFBLK,S_IFCHR,S_IFDIR ...*/
	spinlock_t		i_lock;	/* i_blocks, i_bytes, maybe i_size */
	struct mutex		i_mutex;
	struct rw_semaphore	i_alloc_sem;
	const struct inode_operations	*i_op;
	const struct file_operations	*i_fop;	/* 文件操作函数在这里提供former ->i_op->default_file_ops */
	...
	union {
		...
		struct block_device	*i_bdev;	/*指向块设备的指针，文件所在的文件系统所绑定的块设备*/
		struct cdev		*i_cdev;
	};
...
/*文件对象的作用是描述进程和文件交互的关系,同一个文件在不同的进程中有不同的文件对象*/
struct file {
	/*
	...
	struct path		f_path;
#define f_dentry	f_path.dentry	/*文件对应的dentry结构*/
#define f_vfsmnt	f_path.mnt	/*文件所属于的文件系统的vfsmount对象*/
...
/*超级块代表了整个文件系统自身，超级块是对应文件系统自身的控制结构,顺着super_block链表可以遍历整个操作系统打开过的文件的inode结构*/
...
struct super_block {
	...
	struct list_head	s_inodes;	/*指向文件系统内所有的inode,通过它可以遍历inode对象。 all inodes */
...
extern int get_sb_pseudo(struct file_system_type *, char *,
	...
	struct vfsmount *mnt);
/* 把超级块对象赋给vfsmount结构所指的超级块，同时vfsmount所指的mnt_root点赋值为超级块所指的根dentry*/
