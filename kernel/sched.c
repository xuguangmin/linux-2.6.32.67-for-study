...
struct rq {
	 ...
	 */
	unsigned long nr_running; /* 运行队列链表中可运行进程数量 */
	#define CPU_LOAD_IDX_MAX 5
	unsigned long cpu_load[CPU_LOAD_IDX_MAX]; /*基于运行队列中进程的平均数量的CPU负载因子*/
#ifdef CONFIG_NO_HZ
	 ...
	 */
	 /* 先前在运行队列链表中，而现在睡眠在TASK_UNINTERRUPTIABLE状态的进程数量 */
	unsigned long nr_uninterruptible;

	struct task_struct *curr, *idle;/*当前正在运行进程的进程描述符和当前CPU上swapper进程的进程描述符指针*/
	...
	atomic_t nr_iowait;/*先前在运行队列链表中而现在等待磁盘IO操作结束的进程的数量*/

#ifdef CONFIG_SMP
	struct root_domain *rd;
	struct sched_domain *sd;/*指向当前CPU的进步调度域*/
	       ...
	       struct task_struct *next)
{
	...
	if (unlikely(!mm)) {//如果next是一个内核线程，使用prev的地址空间
		next->active_mm = oldmm;
		atomic_inc(&oldmm->mm_count);
		enter_lazy_tlb(oldmm, next);
	} else
		switch_mm(oldmm, mm, next);//如果是普通进程，用next的地址空间替换prev的地址空间

	if (unlikely(!prev->mm)) {//如果prev是内核线程或正在退出的进程就把指向prev内存描述符的指针保存到运行队列的prev_mm字段中
		prev->active_mm = NULL;
		rq->prev_mm = oldmm;
	}
	/*
 ...
 * this_rq\this_cpu:本地运行队列描述符和本地CPU下标
 * busiest:源运行队列描述符
 * max_load_move:被迁移进程的最大数
 * sd:在其中执行平衡操作的调度域的描述符地址
 * idle:SCHED_IDLE  NOT_IDLE   NEWLY_IDLE
 */
 ...
 * 函数分析调度域中各组的工作量，函数返回最繁忙的
 * 组的sched_group描述符的地址，假设这个组不包括本地CPU
 * 如果最繁忙的组包括本地CPU,或所有的组本来就是平衡的
 * 函数返回NULL
 */
...
redo:
	...
	//查找在上面找到的组中最繁忙的CPU,返回相应运行队列的描述符
	...
	if (!ld_moved) {
		...
		if (unlikely(sd->nr_balance_failed > sd->cache_nice_tries+2)) {
			...
			/* 如果函数move_task没有成功的把某些进程迁移到本地运行
			  * 队列，那么调度域还是不平衡，把busiest->active_balance置为1,
			  * 并唤醒migration内核线程*/
	...
	//调整调度域的参数，以延迟本地CPU下一次对load_balance的调度
		       ...
		       cputime_t cputime_scaled)
{
	...
	p->utime = cputime_add(p->utime, cputime);//用户态下经过的节拍数
...
need_resched_nonpreemptible:
	...
	if (prev->state && !(preempt_count() & PREEMPT_ACTIVE)) {
		if (unlikely(signal_pending_state(prev->state, prev)))
			...
			deactivate_task(rq, prev, 1);//从运行队列删除该进程
	...
	if (unlikely(!rq->nr_running))
		idle_balance(cpu, rq);//如果运行队列没有可运行进程，从另外运行队列迁移一些进程到本地运行队列中 

	put_prev_task(rq, prev);
	next = pick_next_task(rq);//局部变量next现在存放将取代prev的进程描述符指针
 ...
 */
 /*对于遍历过程中的每个节点都会调用该节点上的函数func,唤醒等待的进程,完成接口中使用到*/
 ...
 * changing the task state if and only if any tasks are woken up.
 */
 /*补充原语接口，对应up();另一个是complete_all,前者只唤醒一个等待者，后者将唤醒所有等待者*/
 ...
 */
/* 补充原语对执行路径间的同步可以通过等待者与完成者模型来表述，
 * 对于等待者的行为,内核定义了wait_for_completion函数，对应down()*/
...
recheck:
	 ...
	 */
	if (user && !capable(CAP_SYS_NICE)) {//检查调用进程权能
		if (rt_policy(policy)) {
