...
struct workqueue_struct {
	struct cpu_workqueue_struct *cpu_wq; /*指向cpu工作队列管理结构的per-CPU
			* 根据该指针,系统中每个cpu都可以通过per_cpu_ptr来获得属于自己的cpu工作队列管理结构的对象*/
 ...
 */
 /* 该函数使调用进程一直处于阻塞状态，知道等待队列中的所有函数执行完毕,但是不会等待
  * 在调用flush_workqueue之后加入到等待队列中的函数
 ...
 */
  ...
  * 驱动程序如果使用内核预定义的工作队列只需调用schedule_workqueue就可以了*/
 ...
 * 预定义工作队列函数;等价于queue_delayed_work
 */
...
/*生成一个工作者线程,在linux内核中内核线程其实是一个进程*/
